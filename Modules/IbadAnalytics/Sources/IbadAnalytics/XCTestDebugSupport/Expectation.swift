...
func expectation(description: String = "") -> () -> Void {
#if canImport(ObjectiveC)
    guard
        let currentTestCase = _XCTCurrentTestCase(),
        let expectation = currentTestCase
    ...
    return {
        if !isFulfilled {
            ...
            _ = currentTestCase.perform(Selector(("waitForExpectations:timeout:")), with: [expectation], with: 0)
            isFulfilled = true
        }
    }
#else
...
func expect<R>(action: @escaping () -> R) -> () -> R {
#if canImport(ObjectiveC)
...
func expect<A, R>(action: @escaping (A) -> R) -> (A) -> R {
#if canImport(ObjectiveC)
    let fulfill = expectation(description: "should be called")
    return {
        let result = action($0)
